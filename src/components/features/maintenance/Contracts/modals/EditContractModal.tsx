...
export default function EditContractModal({ contrat, onClose, prestataires, onSave }: EditContractModalProps) {
    ...
    // Initialiser les données du formulaire quand le contrat change
    useEffect(() => {
        if (contrat) {
            setFormData({
                libelle: contrat.nom,
                ...
                taciteReconduction: contrat.taciteReconduction,
                delaiResiliation: contrat.delaiResiliation || 30,
                coutAnnuel: contrat.coutAnnuel.toString(),
                fichierPDF: contrat.fichierPDF || '',
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const updatedContrat: ContratDetaille = {
            ...
            fichierPDF: formData.fichierPDF || contrat.fichierPDF,
            dateUploadPDF: formData.fichierPDF && formData.fichierPDF !== contrat.fichierPDF
                ? new Date().toISOString()
                : contrat.dateUploadPDF,
            estReglementaire: formData.estReglementaire
        };

        onSave(updatedContrat);
        onClose();
    };
                ...
                <form onSubmit={handleSubmit} className={styles.modalContent}>
                        ...
                        <div className={clsx(styles.formGroup, styles.fullWidth)}>
                            ...
                            <div className={styles.fileUpload}>
                                <input
                                    type="file"
                                    accept=".pdf"
                                    id="edit-contrat-pdf-upload"
                                    onChange={e => setFormData({ ...formData, fichierPDF: e.target.files?.[0]?.name || '' })}
                                    className={styles.fileInput}
                                />
                                <label htmlFor="edit-contrat-pdf-upload" className={styles.fileUploadLabel}>
                    ...
                    <div className={styles.modalActions}>
                        ...
                        <button type="submit" className="btn btn-primary">
                            <Check size={16} aria-hidden="true" /> Enregistrer les modifications
                        </button>
                    </div>
                </form>
            </div>
