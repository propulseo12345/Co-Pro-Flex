...
import { getTypeLabel, getStatusBadge } from './helpers';
import clsx from 'clsx';
import styles from './AgOverview.module.css';

interface AgNextMeetingCardProps {
  ag: AgOverview;
  isConvoked: boolean;
}

export function AgNextMeetingCard({ ag, isConvoked }: AgNextMeetingCardProps) {
  const typeLabel = getTypeLabel(ag.meeting_type);
  const statusBadge = getStatusBadge(ag.status, styles);
