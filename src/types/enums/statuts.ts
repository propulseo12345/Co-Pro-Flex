...
/**
 * Statuts possibles d'un appel de fonds
...
/**
 * Type pour le statut d'appel de fonds (alias pour compatibilité)
 */
export type StatutAppelFonds = `${AppelFondsStatut}`;

/**
 * Métadonnées des statuts d'appel de fonds
 */
export const STATUTS_APPEL_FONDS: Record<AppelFondsStatut, {
  label: string;
  labelCourt: string;
  couleur: string;
  icone: string;
  ...
  [AppelFondsStatut.BROUILLON]: {
    label: 'Brouillon',
    labelCourt: 'Brouillon',
    couleur: '#6B7280',
    icone: 'FileEdit',
    description: 'Appel en cours de préparation',
  },
  [AppelFondsStatut.EN_ATTENTE]: {
    label: 'En attente d\'émission',
    labelCourt: 'À émettre',
    couleur: '#F59E0B',
    icone: 'Clock',
    description: 'Appel prêt à être émis aux copropriétaires',
  },
  [AppelFondsStatut.EMIS]: {
    label: 'Émis',
    labelCourt: 'Émis',
    couleur: '#3B82F6',
    icone: 'Send',
    description: 'Appel envoyé aux copropriétaires',
  },
  [AppelFondsStatut.PARTIELLEMENT_PAYE]: {
    label: 'Partiellement payé',
    labelCourt: 'Partiel',
    couleur: '#8B5CF6',
    icone: 'CircleDot',
    description: 'Certains copropriétaires ont réglé',
  },
  [AppelFondsStatut.SOLDE]: {
    label: 'Soldé',
    labelCourt: 'Soldé',
    couleur: '#10B981',
    icone: 'CheckCircle',
    description: 'Tous les copropriétaires ont réglé',
  },
  [AppelFondsStatut.ANNULE]: {
    label: 'Annulé',
    labelCourt: 'Annulé',
    couleur: '#EF4444',
    icone: 'XCircle',
    description: 'Appel annulé',
  },
};

/**
 * Transitions autorisées entre statuts d'appel de fonds
 */
export const APPEL_FONDS_TRANSITIONS: Record<AppelFondsStatut, AppelFondsStatut[]> = {
  [AppelFondsStatut.BROUILLON]: [AppelFondsStatut.EN_ATTENTE, AppelFondsStatut.ANNULE],
  [AppelFondsStatut.EN_ATTENTE]: [AppelFondsStatut.EMIS, AppelFondsStatut.ANNULE],
  [AppelFondsStatut.EMIS]: [AppelFondsStatut.PARTIELLEMENT_PAYE, AppelFondsStatut.SOLDE, AppelFondsStatut.ANNULE],
  [AppelFondsStatut.PARTIELLEMENT_PAYE]: [AppelFondsStatut.SOLDE, AppelFondsStatut.ANNULE],
  [AppelFondsStatut.SOLDE]: [],
  [AppelFondsStatut.ANNULE]: [],
};
 ...
 */
export function peutTransitionnerAppel(
  statutActuel: AppelFondsStatut,
  nouveauStatut: AppelFondsStatut
): boolean {
  return APPEL_FONDS_TRANSITIONS[statutActuel]?.includes(nouveauStatut) ?? false;
}

/**
 * Vérifie si un appel peut être émis
 */
export function peutEmettreAppel(statut: AppelFondsStatut | string): boolean {
  return statut === AppelFondsStatut.EN_ATTENTE;
}

/**
 * Vérifie si un appel peut être modifié
 */
export function peutModifierAppel(statut: AppelFondsStatut | string): boolean {
  return statut === AppelFondsStatut.BROUILLON || statut === AppelFondsStatut.EN_ATTENTE;
}

/**
 * Vérifie si un appel peut être annulé
 */
export function peutAnnulerAppel(statut: AppelFondsStatut | string): boolean {
  return statut !== AppelFondsStatut.SOLDE && statut !== AppelFondsStatut.ANNULE;
}
