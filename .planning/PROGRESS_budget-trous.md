...
- [x] **2026-06-08** — **Trou #1 FRONT livré** (Phase B) : wrappers api + `handleTransferALUR` réel + modale réécrite + bandeau rappel + solde 105 + historique. **Revue ultra front (no-go)** → bloquant = **page doublon `/finance/fonds-alur` supprimée** (`1b2e693`, chemin cassé/hors-compta, non linée nav) + correctifs (anti double-submit, CSS mortes, solde 0, reload onglet, Invalid Date, aria, wording) (`5a20f9c`). tsc 0 + vitest 78/78. **Reste : vérif écran USER.**
- **Différés #1 (notés)** : champ « note/motif » dans la modale (p_notes supporté mais non saisi) ; badge reliant bandeau↔historique (exposer `cash_settled` dans `v_alur_transfers_history`) ; mapping `reserve/other` ; ré-injecter dans l'onglet ALUR les 2 extras de l'ex-page fonds-alur (**recherche lots + export Excel**) si besoin ; a11y transverse des modales (focus-trap/Escape).
