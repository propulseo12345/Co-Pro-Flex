...
- **G2** 🟢 **TRANCHÉ** — **Cible cloud bêta = projet Supabase NEUF** dédié, schéma propre `0001→0044` redéployé. Cloud actuel laissé **intact**. **Toute migration cloud = sur GO explicite.**
  - ✅ **RE-BASELINE PROUVÉE (2026-06-10)** : la chaîne **0001→0044 rejoue à 0 erreur** sur une base neuve (environnement Supabase fidèle) + smoke `audit_finance_integrity=0`. La mémoire « migrations non reproductibles » était **périmée** (d'avant la reconstruction propre). Harnais re-jouable : `scripts/rebaseline-check.sh`. → débloque le déploiement cloud neuf + le passage du job CI `db:test` en **bloquant**.
