-- Lot 1 Phase 2b : open_next_period ventile le résultat 6/7 en 120 (courant) et 110 (travaux).
-- Corps repris de 20260601114000 (WP5.2) ; seul le bloc "résultat" est modifié (split 110/120).
-- Comptes travaux/exceptionnels -> 110 : 671,672,673,674,677,678,702,705,706. Reste 6/7 -> 120.
-- Appliquée via MCP apply_migration le 2026-06-03 (sanity split sur exo 2025 boucle d'or OK).
...
BEGIN
  ...
  IF v_acct_110 IS NULL THEN
    RETURN jsonb_build_object('success', false,
      'error', 'Compte 110 absent pour cette copropriété', 'copro_id', p_copro_id);
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'result_net67', (v_net_courant + v_net_travaux),
