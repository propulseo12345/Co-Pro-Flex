-- 0069_charge_nature_662_courant.sql
-- ============================================================================================
-- Correctif charge_nature : 662 (autres charges financières et agios) -> 'courant'.
--
-- CONTEXTE : 0067 avait classé 661/662/704 en 'travaux'. La revue + le fac-similé officiel de
-- l'annexe 2 (cf. .planning/FACSIMILE_ANNEXES_2026-06-15.md) montrent que le 662 (agios) relève des
-- OPÉRATIONS COURANTES (frais de fonctionnement de la trésorerie), pas des travaux. Décision USER+expert
-- du 2026-06-15 : 662 -> courant. 661 / 704 / 705 RESTENT 'travaux' (financement des travaux : corrects).
--
-- CE QUI EST CORRIGÉ :
--   (1) provision_copro_chart : la branche 'travaux' du seed PERD 662 (futures copros).
--   (2) backfill ciblé des comptes EXISTANTS encore en 'travaux' (copros déjà provisionnées par 0067).
--
-- NON CONCERNÉ (laissé volontairement) :
--   - 703 (avances) RESTE 'courant' : compte contextuel (fonds de roulement = courant ; avance travaux =
--     travaux), à ventiler par operation_id (E4) le jour où un écrivain le poste. Aucun écrivain ne poste
--     703 aujourd'hui -> aucune urgence, "courant" = défaut le moins faux.
--
-- INVARIANTS (ne PAS toucher) :
--   - 711 (subventions) RESTE 'courant' (bascule travaux via operation_id E4).
--   - open_next_period / v_result_allocation_split (0059) NON régénérés (préserve la précédence
--     operation_id câblée en E4/0060). Le correctif de données suffit, la logique de lecture est inchangée.
--   - CHECK ck_accounts_charge_nature (0059) inchangé : 662 est classe 6, 'courant' est autorisé.
-- ============================================================================================

-- ── 1. provision_copro_chart : retire 662 de la branche 'travaux' (futures copros) ──────────────
-- Corps EXTRAIT VERBATIM de 0067 ; SEULE différence : la liste de codes 'travaux' du CASE final PERD
-- 662 (le backfill 0067 n'est PAS rejoué — voir bloc 2 pour les comptes existants).
...
begin
  ...
  select
    ...
    -- E3/0067/0069 : nature de charge dérivée du code (source unique de classification courant/travaux).
    -- 0069 : 662 RETIRÉ de la liste 'travaux' (agios = courant). 661/704/705 restent travaux.
    case when c.code in ('661','671','672','673','674','677','678','6221','702','704','705','706') then 'travaux'
...
-- ── 2. Backfill ciblé des comptes EXISTANTS (copros déjà provisionnées par 0067) ─────────────────
-- Reclasse 662 mal classé en 'travaux' -> 'courant'. Garde `= 'travaux'` : idempotent (ne touche pas un
-- 662 déjà courant). 661/704 restent travaux (NON touchés).
update public.accounts
   set charge_nature = 'courant'
 where code = '662'
   and charge_nature = 'travaux';

-- FIN 0069_charge_nature_662_courant.sql
