-- ============================================================
-- FIX seed_golden_loop — bug latent max(uuid)
-- ============================================================
-- Découvert via le harnais de test V0 : la résolution des 2 clés spéciales
-- faisait `max(id) FILTER (...)` sur repartition_keys.id (uuid). Postgres n'a
-- pas d'agrégat max(uuid) → ERROR 42883 au PLAN → seed_golden_loop cassée pour
-- TOUTE copro (la boucle d'or actuelle avait été seedée par une version
-- antérieure, avant le support multi-clés). Correctif : 2 SELECT LIMIT/OFFSET
-- déterministes (ORDER BY created_at) à la place de l'agrégat. Reste identique.
...
BEGIN
  ...
  -- FIX : 2 clés spéciales résolues par LIMIT/OFFSET (plus de max(uuid))
  ...
  FOR v_lot IN SELECT id FROM lots WHERE copro_id = p_copro_id AND NOT (id = ANY (v_unpaid)) LOOP
    ...
    IF v_due IS NOT NULL AND v_due > 0 THEN
      -- 9e arg (idempotency_key) explicite : désambiguïse les 2 surcharges post_owner_payment (cf. item 5.7)
  ...
  IF (v_inv->>'success')::boolean THEN
    ...
    -- 8e arg (idempotency_key) explicite : désambiguïse les 2 surcharges post_supplier_payment
