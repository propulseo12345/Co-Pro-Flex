-- ============================================================================
-- WP4 (finitions) — Corrections KPI dashboard, révélées par le seed propre
-- ----------------------------------------------------------------------------
-- 1) fn_annexe_2 :
--    a) `all_charge_accounts` / `all_product_accounts` dédoublonnaient par
--       (code, NOM). Or le réalisé nomme un compte via accounts.name et le
--       budget via budget_lines.label → noms différents pour le même code →
--       compte présent 2× → totaux DOUBLÉS (budget voté et réalisé gonflés).
--       Fix : dédoublonner par CODE seul.
--    b) `total_i_charges.ex_clos_realise` (et ex_precedent_approuve) sommaient
--       `product_lines` (produits classe 7, dont 701 = provisions appelées) au
--       lieu de `charge_lines` (charges classe 6). → le « budget réalisé »
--       affichait les provisions. Fix : sommer charge_lines.
-- 2) fn_dashboard_kpis : `total_impayes` lisait les créances totales (annexe 1).
--    Le KPI métier attendu = impayé ÉCHU (appels échus non réglés) → on le
--    branche sur la vue canonique v_unpaid_by_lot.
-- ============================================================================
...
BEGIN
  ...
  -- FIX 1a : dédoublonnage par CODE seul (le nom peut différer entre réalisé et budget)
  all_charge_accounts AS (
    SELECT account_code, MIN(account_name) AS account_name FROM (
  ...
  SELECT jsonb_build_object(
    ...
    -- FIX 1b : « Total I — Charges » = somme des CHARGES (charge_lines), pas des produits
...
BEGIN
  ...
  -- FIX 2 : impayé ÉCHU (appels échus non réglés), pas la créance totale
